...
Viene fornita all'utente l'opportunità di crearsi il proprio dataset di allenamento successivamente usato sulla rete neurale che esso stesso potrà definirsi settando i parametri  opportuni.
...
\item studio teorico CNN (Convolutional Neural Network)
...
Le cause delle crisi epilettiche possono essere diverse, come in caso di patologie,  un trauma cranico, infezioni SNC (meningite ecc..), ed anche a causa di vari farmaci che possono essere somministrati.
...
\item Complessa: Interessa aree più vaste del cervello e non è concentrata solo su alcune di esse. Questo tipo di crisi è più dannosa perchè scaturisce nel soggetto coinvolto una perdita di conoscenza.
\end{itemize}
...
\end{itemize}
Prima di affermare con certezza di essere effetti da epilessia bisogna essere colpiti non da una crisi epilettica isolata, che può avvenire anche in termini molto brevi nella vita di tutte le persone, ma bisogna essere colpiti più volte e in modo ripetuto da più crisi epilettiche prima di affermare di essere affetti da epilessia.
...
La diagnosi dell'epilessia viene fatta attraverso diversi strumenti, uno su tutti è l'elettroencefalogramma (EEG). Questo strumento registra le attività elettriche dell'encefalo, poi riprodotte su dei tracciati grafici, che mostrano delle onde, corrispondenti agli elettrodi applicati sullo scalpo del soggetto.
Questi tracciati riprodotti sono il risultato della differenza di potenziale tra un elettrodo e un altro applicati in diverse zone dello scalpo.
Attraverso l'elettroencefalogramma è possibile individuare le cause di una crisi epilettica e le alterazioni  elettriche che avvengono all'interno del cervello sia in condizioni normali, di riposo, che in situazioni di crisi. In base alle caratteristiche del tracciato risultante i medici possono dedurre di che tipo di epilessia si tratta e quale cura è meglio prescrivere. 
https://www.my-personaltrainer.it/salute/epilessia.html
\todo{ spiegare frequenza eeg e loro composizione}
\todo{ aggiungere foto dello scalpo con elettrodi oppure di un tracciato EEG   }
...
\todo{ si potrebbe aggiungere la funzione di softmax e spiegare il fully connected che avviene il reshape con un immagine }
...
\todo{si potrebbe spiegare meglio la relu come spiegato per la sigmoide sopra}
...
\todo{ DA VERIFICARE LA PARTE SOTTOSTANTE, VEDERE DOVE METTERE QUESTA SEZIONE}
...
\item sezione con relativo grafico che mostra il numero totale di valori registrati all'interno del tracciato ( PER un singolo CANALE O TUTTI ???)
...
\item setting della rete con tutti i relativi parametri, possibilità di creare piu livelli convoluzionali, ma è possibile solo aggiungere un livello lineare
\item infine vengono settate le impostazioni di allenamento della rete, attraverso l'inserimento del numero di epoche e il modo in cui viene fatto tale allenamento (cross-validation)
\end{itemize}
\end{itemize}
