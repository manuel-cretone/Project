...
def createDataset(filename, seizureStart, seizureEnd, windowSize, stride):
    ...
    for channel in range(23):
        try:
            seizureSignals, normalSignals = getDataset(""+filename+".edf", channel, seizureStart, seizureEnd, windowSize, stride)
            signals = np.concatenate((seizureSignals, normalSignals))
            df = pd.DataFrame(data = signals)
            df.to_csv("./csv_files/"+filename+'/chn'+channel.__str__()+'.csv', index=False, header=False)
        except:
